...
def _cache_dataset_state(
    ...
    # FIX 4: result.model_dump() previously included the full `data`
    # field (all dataset rows) inside LAST_SCAN_STATUS. This doubles
    # the session state size because FINAL_RECORDS already holds the
    # same rows. On large datasets this pushes state well past Vertex
    # AI Agent Engine limits, causing blank responses and stream errors.
    # We exclude `data` from the serialized snapshot stored here.
    tool_context.state[LAST_SCAN_STATUS] = (
        result.model_dump(exclude={"data"})
    )

    # FINAL_RECORDS is the single authoritative copy of the dataset rows.
    # It is stored once and never duplicated elsewhere in session state.
...
def _load_and_cache_master_dataset(
    ...
    Returns:
        DatasetResponse:
            Dataset load result WITHOUT the raw data rows.
            The rows are stored in session state under FINAL_RECORDS only.
    """
    ...
    try:
        ...
        # FIX 4: Build the DatasetResponse with data=[] (empty).
        # The actual rows are passed separately to _cache_dataset_state
        # which stores them under FINAL_RECORDS in session state.
        # Putting thousands of rows into the DatasetResponse object
        # would cause them to be serialized a second time by ADK when
        # output_key="onboarding_result" writes this object back to
        # session state — doubling memory and blowing past state limits.
        result = DatasetResponse(
            status="success",
            ...
            data=[],  # FIX 4: intentionally empty — rows live in FINAL_RECORDS only
        )
...
def continue_with_existing_dataset(
    ...
    Returns:
        DatasetResponse:
            ...
            The data field is always empty; rows are in session state.
    """
