# Distribution Analaysis Agent

A Python-based agent that helps creates and share performance analysis report using Google's Agent Development Kit (ADK) and Vertex AI.
...
```bash
cd performance-management-adk-deployed
```

2. Install Poetry if you haven't already:
```bash
curl -sSL https://install.python-poetry.org | python3 -
curl --ssl-no-revoke -sSL https://install.python-poetry.org | python3 -
curl --ssl-no-revoke -sSL https://install.python-poetry.org | "C:\Users\prebansal\AppData\Local\Programs\Python\Python312\python.exe" -

```

3. Install project dependencies:
```bash
if no pyproject.toml file-
poetry init
poetry install
C:\Users\neejha\AppData\Roaming\Python\Scripts\poetry.exe install
```

4. Activate the virtual environment:
```bash
source $(poetry env info --path)/bin/activate
source $(C:\Users\neejha\AppData\Roaming\Python\Scripts\poetry.exe poetry env info --path)/bin/activate
```
